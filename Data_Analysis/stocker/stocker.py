...
# 完全移除 yfinance 和 quandl，使用多数据源替代
import pandas as pd
import numpy as np
# import fbprophet
import prophet
...
# 导入多数据源模块（遵循标准导入方式规范）
from .data_sources import MultiSourceDataFetcher, safe_get_stock_data
...
class Stocker:
    ...
    def __init__(self, ticker, period="max"):
        ...
        # 使用多数据源获取金融数据（遵循金融数据源选择规范和API调用与错误处理规范）
        try:
            print(f"🔍 开始获取 {ticker} 的股票数据...")
            
            # 使用多数据源获取器
            fetcher = MultiSourceDataFetcher()
            stock, data_source = fetcher.fetch_stock_data(ticker, period)
            
            print(f"✅ 成功获取 {ticker} 数据，数据源: {data_source}，共 {len(stock)} 条记录")
            
        except Exception as e:
            print(f"❌ 多数据源获取失败: {str(e)}")
            print("🔄 使用安全降级策略...")
            
            # 使用安全调用包装函数作为最后的降级策略
            stock = safe_get_stock_data(ticker, period, fallback_to_mock=True)
            
            if stock is None:
                raise ValueError(f"无法通过任何方式获取 {ticker} 的数据")
            
            print(f"📊 使用降级策略成功获取数据，共 {len(stock)} 条记录")

        # 数据已经通过 MultiSourceDataFetcher 标准化，直接使用
